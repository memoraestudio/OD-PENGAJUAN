...
<script type="text/javascript">
    fetchAllSppTransferCek();
    function fetchAllSppTransferCek(){
        $.ajax({
        type: "GET",
        url: "{{ route('bod_spp_transfer_cek/getDataSppTransferCek.getDataSppTransferCek') }}",
        dataType: "json",
        success: function(response) {
            ...
            tabledata += `<tr>`;
                ...
                tabledata += `<td>${chq.no_spp}</td>`;
                tabledata += `<td>${chq.tgl_spp}</td>`;
                tabledata += `<td>${chq.keterangan}</td>`;
                tabledata += `<td>${chq.kode_perusahaan_tujuan}</td>`;
                tabledata += `<td>${chq.nama_bank}</td>`;
                tabledata += `<td></td>`;
                tabledata += `<td>${chq.pembayaran}</td>`;
                tabledata += `<td></td>`;
                tabledata += `<td></td>`;
                tabledata += `<td></td>`;
                tabledata += `<td></td>`;
...
@section('title')
    <title>SPP - cheque - Transfer</title>
@endsection
...
<main class="main">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">Status Kategori SPP</li>
        <li class="breadcrumb-item active">SPP - Cheque - Transfer</li>
    </ol>
                                ...
                                <table id="datatabel-v1" class="table table-bordered table-sm" style="white-space: nowrap; width:100%; margin-bottom: 0;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Tujuan Rekening Perusahaan</th>
                                            <th>Tujuan Vendor</th>
                                            <th>Status Bank (Dokumen Bank)</th>
                                            <th>Pelaku Input Upload</th>
                                            <th>Pelaku Input Otorisasi</th>
                                        </tr>
                                    </thead>
                                    <tbody id="tabledata">
