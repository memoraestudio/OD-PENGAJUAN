                ...
                <div class="col-md-12">
                    <div class="card card-accent-primary">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('sppd_masuk/cari.cari') }}" method="get">
                            ...
                            <div class="table-responsive">
                                ...
                                <div style="width:100%;">
                                    <table class="table table-bordered table-striped table-sm">
                                        ...
                                        <tbody>
                                            <?php $i=1 ?>
                                            @forelse($masuk_sppd as $val)
                                                @if(Auth::user()->kode_divisi == '16') <!-- Jika Biaya-->
                                                 @if(Auth::user()->type == 'Admin') <!-- Jika Biaya-->   
                                                    <tr>
                                                        <td>{{ $i }}</td>
                                                        ...
                                                        <td>{{ $val->sifat }}</td>
                                                        <td align="center">
                                                            @if($val->status_validasi_adm_biaya == '0')
                                                                <label class="badge badge-secondary">New</label>
                                                            @elseif($val->status_validasi_adm_biaya == '1')
                                                                <label class="badge badge-success">Verified</label>
                                                            @elseif($val->status_validasi_adm_biaya == '2')
                                                                <label class="badge badge-danger">Denied</label>
                                                            @elseif($val->status_validasi_adm_biaya == '3')
                                                                <label class="badge badge-warning">Pending</label>
                                                            @endif
                                                        </td>
                                                        <td align="center">   
                                                            <a href="{{ route('sppd_masuk/view.view', $val->kode_pengajuan_sppd)  }}" class="btn btn-primary btn-sm">View</a>    
                                                        </td>
                                                    </tr>
                                                    <?php $i++ ?>
                                                 @elseif(Auth::user()->type == 'Manager') <!-- Jika Biaya-->
                                                    <tr>
                                                        <td>{{ $i }}</td>
                                                        <td>{{ $val->kode_pengajuan_sppd }}</td>
                                                        <td>{{ $val->tgl_pengajuan_sppd }}</td>
