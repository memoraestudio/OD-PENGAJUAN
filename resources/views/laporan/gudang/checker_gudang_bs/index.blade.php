...
<main class="main">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Checker BS</li>
    </ol>
                ...
                <div class="col-md-12">
                    <div class="card card-accent-primary">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('l_checker_gudang_bs/cari.cari') }}" method="get">
                                ...
                                <div class="input-group mb-3 col-md-10 float-right"> 
                                    ...
                                    <select name="id_checker" id="id_checker" class="form-control" required>
                                        ...
                                        @foreach ($checker as $row)
                                            <option value="{{ $row->id_checker_bs }}" {{ old('id_checker_bs') == $row->id_checker_bs ? 'selected':'' }}>{{ $row->nama_checker }}</option>
                                        @endforeach 
                                    </select>
                                    &nbsp
                                ...
                                <div style="width:100%;">
                                    <table class="table table-bordered table-striped table-sm">
                                        ...
                                        @forelse ($data as $val)
                                            <tr>
                                                <td hidden>{{ $no++ }}</td>
                                                <td hidden>{{ $val->id_checker_bs }}</td>
                                                ...
                                                <td align="right">{{ $val->qty_bs}}</td>
                                                <td align="right">{{ $val->qty_ekspedisi}}</td>   
                                            </tr>
                                            @empty
                                            <tr>
        ...
        $(function(){
            $('#kode_perusahaan').change(function(){
                ...
                if(perusahaan_id){
                    $.ajax({
                        type:"GET",
                        url:"/ajax_depo_laporan_bs?perusahaan_id="+perusahaan_id,
                        dataType:'JSON',
        ...
        $(function(){
            $('#kode_depo').change(function(){
                ...
                if(depo_id){
                    $.ajax({
                        type:"GET",
                        url:"/ajax_checker_laporan_bs?depo_id="+depo_id,
                        dataType:'JSON',
                        success: function(res){
                            if(res){
                                $("#id_checker").empty();
