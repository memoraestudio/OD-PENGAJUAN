...
<main class="main">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Izin E</li>
    </ol>
                ...
                <div class="col-md-12">
                    <div class="card card-accent-primary">
                        <div class="card-header">
                            <h4 class="card-title">
                                Izin E
                                <a href="{{ route('tanda_terima_e.create') }}" class="btn btn-primary btn-sm float-right">Buat Pengajuan Izin E</a>
                            </h4>
                        </div>
                        <br>
                                                ...
                                                <table id="datatabel-v1" class="table table-bordered table-sm" style="white-space: nowrap; width:100%; margin-bottom: 0;">
                                                    ...
                                                    <tbody id="tabledata">
                                                        ...
                                                        @forelse($data_pengiriman_cek as $val)
                                                            <tr>
                                                                <td>{{ $no }}</td>
                                                                <td>{{ $val->kode_izin_e }}</td>
                                                                <td>{{ date('d-M-Y', strtotime($val->tgl_kirim)) }}</td>
                                                                <td>{{ $val->no_izin_e }}</td>
                                                                <td>{{ $val->judul_izin_e }}</td>
                                                                <td align="right">{{ $val->jml_lembar }} lembar</td>
                                                                <td align="right">{{ $val->total_belum_terima }} lembar</td>
                                                                <td>{{ $val->yang_ttd }}</td>
                                                                <td hidden>{{ $val->id_user_input }}</td>
                                                                <td>{{ $val->name }}</td>
                                                                <td hidden>{{ $val->no_urut }}</td>
                                                                <td align="center"><a href="{{ route('tanda_terima_e.view', $val->no_urut) }}" class="btn btn-success btn-sm">Terima</a></td>
                                                            </tr> 
                                                            <?php $no++ ?>
                                                        @empty
                                                ...
                                                <table id="datatabel-v1" class="table table-bordered table-sm" style="white-space: nowrap; width:100%; margin-bottom: 0;">
                                                    ...
                                                    <tbody id="tabledata">
                                                        ...
                                                        @forelse($data_terima_cek as $val)
                                                            <tr>
                                                                <td>{{ $no }}</td>
                                                                <td>{{ $val->kode_terima }}</td>
                                                                <td>{{ date('d-M-Y', strtotime($val->tgl_terima)) }}</td>
                                                                <td>{{ $val->kode_kirim }}</td>
                                                                <td>{{ date('d-M-Y', strtotime($val->tgl_kirim)) }}</td>
                                                                <td>{{ $val->no_izin_e }}</td>
                                                                <td>{{ $val->judul_izin_e }}</td>
                                                                <td align="right">{{ $val->jml_lembar }} lembar</td>
                                                                ...
                                                                <td>{{ $val->name }}</td>
                                                                <td hidden>{{ $val->no_urut }}</td>
                                                                <td align="center"><a href="{{ route('tanda_terima_e/pdf', $val->no_urut) }}" target="_blank" class="btn btn-success btn-sm">Cetak</a></td>
                                                            </tr> 
                                                            <?php $no++ ?>
                                                        @empty
