...
class PengajuanUploadOtorisasi extends Controller
{
    public function index()
    {
        ...
        return view ('pengajuan.pengajuan_upload_otorisasi.index', compact('approval_cost'));
    }
    ...
    public function cari()
    {
        ...
        $approval_cost = DB::table('pengajuan_biaya')
                ...
                ->select('pengajuan_biaya.kode_pengajuan_b','pengajuan_biaya.tgl_pengajuan_b','pengajuan_biaya.kategori','pengajuan_biaya.tipe','pengajuan_biaya.kode_perusahaan','perusahaans.nama_perusahaan','pengajuan_biaya.kode_depo','depos.nama_depo','pengajuan_biaya.kode_divisi','divisi.nama_divisi','pengajuan_biaya.keterangan',DB::raw('SUM(pengajuan_biaya_detail.tharga) AS total'),'users.name','pengajuan_biaya.status_atasan','pengajuan_biaya.status_biaya_pusat','pengajuan_biaya.status_biaya','pengajuan_biaya.no_urut','ms_pengeluaran.nama_pengeluaran','ms_pengeluaran.sifat','pengajuan_biaya.status_validasi','pengajuan_biaya.status_validasi_acc','pengajuan_biaya.status_validasi_fin','pengajuan_biaya.status_validasi_fin_upload','pengajuan_biaya.status_validasi_clm')
                ->WhereBetween('pengajuan_biaya.tgl_pengajuan_b', [$date_start,$date_end])
                ->WhereIn('pengajuan_biaya.kategori', ['1','2','3','4','5','130']) //Kode Gaji,mitra,BPJS,insentif
                // ->Where('pengajuan_biaya.status_spp_1', 1)
                // ->Where('pengajuan_biaya.status_spp_2', 1)
                ->Where('pengajuan_biaya.status_atasan', 1)
                ->where('pengajuan_biaya.status_fin', 1)
    ...
    public function view($no_urut)
    {
        $approval_cost_head = DB::table('pengajuan_biaya')
        	...
        	->join('users','pengajuan_biaya.id_user_input','=','users.id')
            ->join('ms_pengeluaran','pengajuan_biaya.kategori','=','ms_pengeluaran.id')
        	->select('pengajuan_biaya.kode_pengajuan_b','pengajuan_biaya.tgl_pengajuan_b','pengajuan_biaya.kategori','pengajuan_biaya.tipe','pengajuan_biaya.kode_perusahaan','perusahaans.nama_perusahaan','pengajuan_biaya.kode_depo','depos.nama_depo','pengajuan_biaya.kode_divisi','divisi.nama_divisi','pengajuan_biaya.keterangan',DB::raw('SUM(pengajuan_biaya_detail.tharga) AS total'),'users.name','pengajuan_biaya.status_atasan','pengajuan_biaya.status_biaya_pusat','pengajuan_biaya.status_biaya','pengajuan_biaya.no_urut','ms_pengeluaran.nama_pengeluaran','ms_pengeluaran.sifat','pengajuan_biaya.status_validasi','pengajuan_biaya.status_validasi_acc','pengajuan_biaya.status_validasi_fin','pengajuan_biaya.status_validasi_fin_upload','pengajuan_biaya.status_validasi_clm','pengajuan_biaya.kategori')
    	...
    	return view('pengajuan.pengajuan_upload_otorisasi.view', compact('approval_cost_head','approval_cost_detail','approval_cost_upload','approval_cost_total'));
    }
    ...
    public function upload(Request $request)
    {
        ...
        $upload = DB::table('pengajuan_biaya')->where('no_urut', $no_urut)
                ->update([
                    'status_validasi_fin_upload' => 1,
                ]);
        ...
        if($request->hasfile('filename')) { 
            foreach ($request->file('filename') as $file) {
                if ($file->isValid()) {
                    ...
                    $insert_upload = DB::table('pengajuan_upload_otorisasi')
                                    ->insert([
                                        'kode_pengajuan_upload' => $kode_pengajuan_b,
        ...
        return redirect()->route('pengajuan_biaya_upload.index');
    }
