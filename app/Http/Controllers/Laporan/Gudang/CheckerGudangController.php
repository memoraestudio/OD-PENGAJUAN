...
class CheckerGudangController extends Controller
{
	public function ajax_depo_laporan(Request $request) // dropdown perusahaan dan depo
    {
    ...
    public function ajax_checker_laporan(Request $request)
    {
    	if(Auth::user()->kode_divisi == '22'){ //Divisi Gudang Pusat
    		$depochecker = DB::table('barang_dagang_in_history_update')
    					...
    					->pluck('barang_dagang_in_history_update.id_checker','checker.nama_checker');
    ...
    public function index(Request $request)
    {
        ...
        if(Auth::user()->kode_divisi == '20'){
        	$checker = DB::table('barang_dagang_in_history_update')
    					->join('checker','barang_dagang_in_history_update.id_checker','=','checker.id_checker')
    					->select('barang_dagang_in_history_update.id_checker','checker.nama_checker','barang_dagang_in_history_update.kode_depo')
       ...
       if(Auth::user()->kode_divisi == '20'){ 
       		$data = DB::table('barang_dagang_in_history_update')
	       		...
	       		->select('barang_dagang_in_history_update.id_checker','checker.nama_checker','barang_dagang_in_history_update.tanggal','perusahaans.nama_perusahaan','depos.nama_depo','barang_dagang_in_history_update.kategori','product_dagang.kode_produk','product_dagang.nama_produk',DB::raw("SUM(barang_dagang_in_detail_history_update.qty_layak) AS qty_layak"),
	       		...
	       		->Where('barang_dagang_in_history_update.kode_depo', Auth::user()->kode_depo)
	       		->groupBy('barang_dagang_in_history_update.id_checker','checker.nama_checker','barang_dagang_in_history_update.tanggal','barang_dagang_in_history_update.kategori','product_dagang.kode_produk','product_dagang.nama_produk','perusahaans.nama_perusahaan','depos.nama_depo')
	       		->get();
       ...
       if(Auth::user()->kode_divisi == '20'){ 
        	return view('laporan.gudang.checker_gudang.index', compact('perusahaan','data','checker'));
        }elseif(Auth::user()->kode_divisi == '22'){ //Divisi Gudang Pusat
        	return view('laporan.gudang.checker_gudang.index', compact('perusahaan','data'));
        }
