...
class OtorisasiTransferClaimTgsmPayController extends Controller
{
    public function index()
    {
        return view ('bod.otorisasi_transfer_claim_tgsm_p.index');
    }
    ...
    public function showDetail($no_urut_pengajuan_biaya)
    {
        ...
        return view('bod.otorisasi_transfer_claim_tgsm_p.detail', compact('pengajuan_biaya_head','pengajuan_biaya_detail','pengajuan_biaya_detail_total','pengajuan_biaya_upload','approval_upload'));
    }
    ...
    { 
        $data_list_otorisasi = DB::table('data_transfer')
                ...
                ->where('data_transfer.norek', '438 018 9467') //TGSM Payrol
                ->where('data_transfer.status_otorisasi', 0)
                ->orderBy('data_transfer.kode_pengajuan_b', 'desc')
                ->orderBy('data_transfer.nama_outlet', 'desc')
                ->get();

        return view('bod.otorisasi_transfer_claim_tgsm_p.view', compact('data_list_otorisasi'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        if (!isset($request->value)) {
            $data_otorisasi
                        ->where('data_transfer.norek', '438 018 9467'); //TGSM Payroll
