...
class OtorisasiTransferClaimLpPayController extends Controller
{
    public function index()
    {
        return view ('bod.otorisasi_transfer_claim_lp.index');
    }
    ...
    public function showDetail($no_urut_pengajuan_biaya)
    {
        ...
        return view('bod.otorisasi_transfer_claim_lp.detail', compact('pengajuan_biaya_head','pengajuan_biaya_detail','pengajuan_biaya_detail_total','pengajuan_biaya_upload','approval_upload'));
    }
    ...
    { 
        $data_list_otorisasi = DB::table('data_transfer')
                ...
                ->join('pengajuan_biaya','data_transfer.kode_pengajuan_b','=','pengajuan_b.kode_pengajuan_b')
                ->where('data_transfer.norek', '438 302 6787') //LP Payroll
                ->where('data_transfer.status_otorisasi', 0)
                ->orderBy('data_transfer.kode_pengajuan_b', 'desc')
                ->orderBy('data_transfer.nama_outlet', 'desc')
                ->get();

        return view('bod.otorisasi_transfer_claim_lp.view', compact('data_list_otorisasi'));
    }
