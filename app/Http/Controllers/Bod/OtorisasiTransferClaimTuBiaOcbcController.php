...
class OtorisasiTransferClaimTuBiaOcbcController extends Controller
{
    public function index()
    {
        return view ('bod.otorisasi_transfer_claim_tu_b_ocbc.index');
    }
    ...
    { 
        $data_list_otorisasi = DB::table('data_transfer')
                ...
                ->where('data_transfer.norek', '010 800 030750') //TA Biaya OCBC biaya
                ->where('data_transfer.status_otorisasi', 0)
                ->orderBy('data_transfer.kode_pengajuan_b', 'desc')
                ->orderBy('data_transfer.nama_outlet', 'desc')
                ->get();

        return view('bod.otorisasi_transfer_claim_tu_b_ocbc.view', compact('data_list_otorisasi'));
    }
