...
class OtorisasiTransferClaimTuaBiaOcbcController extends Controller
{
    public function index()
    {
        return view ('bod.otorisasi_transfer_claim_tua_b_ocbc.index');
    }
    ...
    public function showDetail($no_urut_pengajuan_biaya)
    {
        ...
        return view('bod.otorisasi_transfer_claim_tua_b_ocbc.detail', compact('pengajuan_biaya_head','pengajuan_biaya_detail','pengajuan_biaya_detail_total','pengajuan_biaya_upload','approval_upload'));
    }
    ...
    { 
        $data_list_otorisasi = DB::table('data_transfer')
                ...
                ->where('data_transfer.norek', '010 800 023086') //TUA Biaya ocbc
                ->where('data_transfer.status_otorisasi', 0)
                ->orderBy('data_transfer.kode_pengajuan_b', 'desc')
                ->orderBy('data_transfer.nama_outlet', 'desc')
                ->get();

        return view('bod.otorisasi_transfer_claim_tua_b_ocbc.view', compact('data_list_otorisasi'));
    }
    ...
    {
        $data_view_program_all = DB::select("SELECT pengajuan_biaya.kode_pengajuan_b,
        import_pencapaian_program_detail.tgl_import,
        ...
        INNER JOIN import_pencapaian_program_header ON pengajuan_biaya.no_urut = import_pencapaian_program_header.no_urut_pengajuan
            AND pengajuan_biaya.no_surat_program = import_pencapaian_program_header.no_surat
        INNER JOIN import_pencapaian_program_detail ON import_pencapaian_program_header.no_surat = import_pencapaian_program_detail.no_surat
            AND import_pencapaian_program_header.kode_perusahaan = import_pencapaian_program_detail.kode_perusahaan
