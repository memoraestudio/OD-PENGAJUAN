...
class DueDateDtsController extends Controller
{
    public function index()
    {
    	...
    	return view ('accounting.due_date_dts.index', compact('perusahaan_dms','depo_dms','jt'));
    }
    ...
    public function duedatecari(Request $request)
    {
        ...
        if(request()->customer != '' and request()->docId == '' ){ //jika customer tidak kosong dan docId kosong
            $getRow = DB::connection('sqlsrv_dts')
                    ->table('dms_ar_arinvoice')
        ...
        return view ('accounting.due_date_dts.index', compact('jt'));
    }
        ...
        if($validator->passes()){
            foreach ($request->input("szDocId") as $key => $value) {
                if($request->get("tanggal")[$key] != ''){
                    ...
                    //update due_date DMS
                    $update_duedate = DB::connection('sqlsrv_dts')->table('dms_ar_arinvoice')
