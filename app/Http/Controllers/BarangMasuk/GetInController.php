...
class GetInController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view ('masuk_barang.checker.index', compact('in'));
    }
    ...
    public function view($doc_id)
    {
        ...
        return view ('masuk_barang.checker.view', compact('head','detail'));  
    }

    public function ajax_zona_primary_layak(Request $request) // dropdown perusahaan dan depo
    {   
    ...
    public function ajax_zona_primary_bs(Request $request) // dropdown perusahaan dan depo
    {
    ...
    public function ajax_zona_secondary_layak(Request $request) // dropdown perusahaan dan depo
    { 
        if(Auth::user()->kode_depo == '902'){
    ...
    public function ajax_zona_secondary_bs(Request $request) // dropdown perusahaan dan depo
    {
    ...
    public function create(Request $request)
    {
    	...
    	return view('masuk_barang.checker.create', compact('checker_layak','checker_bs','area','area_bs')); //,'sub_area'
    }
    ...
    public function store(Request $request)
    {
        ...
        if(request()->kategori == 'Primary'){
            ...
            BarangDagang_In_History::create([
                'doc_id' => $request->get('surat_jalan'),
                'tanggal' => Carbon::now()->format('Y-m-d'),
                'waktu' => Carbon::now()->format('H:i:s'),
                'kode_perusahaan' => Auth::user()->kode_perusahaan,
            ...
            if($validator){
                foreach ($request->input("kode_produk") as $key => $value) {
                    $data = new BarangDagang_In_Detail;
                ...
                foreach ($request->input("kode_produk") as $key => $value) {
                    $data = new BarangDagang_In_History_Detail;
        ...
        return redirect()->route('get_in.index');
    }
