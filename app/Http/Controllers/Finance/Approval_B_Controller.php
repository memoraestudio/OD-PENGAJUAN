...
class Approval_B_Controller extends Controller
{
    public function index()
    {
        ...
        if(Auth::user()->kode_divisi == '5'){ //-- Jika Finance--
            $data_approval_B =  DB::table('izin_b')
                ...
                ->select('izin_b.kode_izin_b','izin_b.tgl_izin_b','izin_b.no_izin_b','izin_b.judul_izin_b',
                        'izin_b.rekening_pembayar','banks.nama_bank','rekening_fin_comp.atas_nama_rek','izin_b.no_urut','izin_b.status_mengetahui_1')
            ...
            $data_approval_B =  DB::table('izin_b')
                ...
                ->join('banks','rekening_fin_comp.kode_bank','=','banks.kode_bank')
                ->select('izin_b.kode_izin_b','izin_b.tgl_izin_b','izin_b.no_izin_b','izin_b.judul_izin_b',
                        'izin_b.rekening_pembayar','banks.nama_bank','rekening_fin_comp.atas_nama_rek','izin_b.no_urut','izin_b.status_mengetahui_1','izin_b.status_approval')
                ->where('izin_b.tgl_izin_b.status_mengetahui_1', 1)
                ->WhereBetween('izin_b.tgl_izin_b', [$date_start,$date_end])
                ->get();
        }

    	return view('finance.approval_b.index', compact('data_approval_B'));
    }
    ...
    {
    	$head = DB::table('izin_b')
                ->join('rekening_fin_com','izin_b.rekening_pembayar','=','rekening_fin_com.norek')
                ->select('izin_b.kode_izin_b','izin_b.tgl_izin_b','izin_b.no_izin_b','izin_b.judul_izin_b',
                        'rekening_fin_comp.atas_nama_rek','izin_b.rekening_pembayar','izin_b.catatan_b')
                ->Where('izin_b.no_urut', $no_urut)
                ->first();

        $detail = DB::table('tanda_terima_cek')
        ...
        return view('finance.approval_b.view',compact('head','detail','total_jml','count'));
    }
    ...
    public function approved(Request $request)
    {
        if(Auth::user()->kode_divisi == '5'){ //-- Jika Finance--
            ...
            return redirect()->route('approval_b.index');
        }elseif(Auth::user()->kode_divisi == '14'){ //-- BOD--
            $approved = DB::table('tanda_terima_cek')
                        ->select('tanda_terima_cek.receipt_id')
                        ->Where('tanda_terima_cek.receipt_id', $request->get("receipt_id"))
