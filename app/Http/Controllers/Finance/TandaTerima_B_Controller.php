...
use App\Izin_B;
use App\Izin_B_Detail;
use Carbon\carbon;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Barryvdh\DomPDF\Facade as PDF;

class TandaTerima_B_Controller extends Controller
{
    public function index()
    {
        ...
        $data_izin_b = DB::table('izin_b')
                    ...
                    ->select('izin_b.kode_izin_b','izin_b.tgl_izin_b','izin_b.no_izin_b','izin_b.judul_izin_b',
                            'izin_b.rekening_pembayar','banks.nama_bank','rekening_fin_comp.atas_nama_rek','izin_b.no_urut')
                    ->WhereBetween('izin_b.tgl_izin_b', [$date_start,$date_end])
                    ->get();

        return view('finance.tanda_terima_cek_giro_b.index', compact('data_izin_b'));
    }
    ...
    {
        $header = DB::table('izin_b')
                    ->join('rekening_fin_comp','izin_b.rekening_pembayar','=','rekening_fin_comp.norek')
                    ->join('banks','rekening_fin_comp.kode_bank','=','banks.kode_bank')
                    ->join('users','izin_b.id_user_input','=','users.id')
                    ->leftJoin('users as mengetahui','izin_b.id_user_mengetahui_1','=','mengetahui.id')
                    ->leftJoin('users as approved','izin_b.id_user_approval','=','approved.id')
                    ->select('izin_b.kode_izin_b','izin_b.tgl_izin_b','izin_b.no_izin_b','izin_b.judul_izin_b',
                            'izin_b.rekening_pembayar','banks.nama_bank','rekening_fin_comp.atas_nama_rek','izin_b.no_urut',
                            'izin_b.id_user_input','users.name','izin_b.catatan_b',
                            'izin_b.id_user_mengetahui_1','mengetahui.name as mengetahui','izin_b.tgl_approval_1',
                            'izin_b.id_user_approval','approved.name as approved','izin_b.tgl_approval')
                    ->Where('izin_b.no_urut', $no_urut)
                    ->first();
        
        $detail_b = DB::table('izin_b')
                    ->join('izin_b_detail','izin_b.kode_izin_b','=','izin_b_detail.kode_izin_b')
                    ->join('rekening_fin','izin_b_detail.kode_vendor','=','rekening_fin.kode_vendor')
                    ->join('vendors','rekening_fin.kode_vendor','=','vendors.kode_vendor')
                    ->join('banks','izin_b_detail.kode_bank_vendor','=','banks.kode_bank')
                    ->select('izin_b.kode_izin_b','izin_b_detail.id_cek','izin_b_detail.keterangan',
                            DB::raw('COUNT(izin_b_detail.keterangan) AS jml'),
                            'izin_b_detail.kode_seri_warkat','izin_b_detail.seri_awal','izin_b_detail.seri_akhir',
                            'izin_b_detail.kode_vendor','vendors.nama_vendor','izin_b_detail.atas_nama','izin_b_detail.kode_bank_vendor',
                            'banks.nama_bank','izin_b_detail.no_rekening_vendor')
                    ->groupBy('izin_b.kode_izin_b','izin_b_detail.id_cek', 'izin_b_detail.keterangan','izin_b_detail.kode_seri_warkat',
                            'izin_b_detail.seri_awal','izin_b_detail.seri_akhir','izin_b_detail.kode_vendor','vendors.nama_vendor',
                            'izin_b_detail.atas_nama','izin_b_detail.kode_bank_vendor','banks.nama_bank',
                            'izin_b_detail.no_rekening_vendor')
                    ->where('izin_b.no_urut', $no_urut)
                    ->get();            
        
        $total_jml = DB::table('izin_b_detail')
                        ->select(DB::raw('count(izin_b_detail.kode_izin_b) as total'))
                        ->where('izin_b_detail.no_urut', $no_urut) 
                        ->first();           

        $pdf = PDF::loadview('finance.tanda_terima_cek_giro_b.pdf', compact('header','detail_b','total_jml'))->setPaper('a4', 'landscape'); //landscape,portrait
        return $pdf->stream();
    }
    ...
    //     return view('finance.tanda_terima_cek_giro_b.view',compact('tanda_terima_head','tanda_terima_detail','total_jml','count'));
    // }
    ...
    { 
    	...
    	return view('finance.tanda_terima_cek_giro_b.create', compact('seri_warkat','perusahaan'));
    }
    ...
    public function actionPembayar(Request $request)
    {
        if($request->ajax())
        {
            $output = '';
            ...
            if($query != ''){
                $data = DB::table('izin_h_detail')
                        ->select(
                            'izin_h_detail.kode_perusahaan',
                            'perusahaans.nama_perusahaan',
                            'rekening_fin_comp.atas_nama_rek',
                            'izin_h_detail.kode_bank',
                            'banks.nama_bank',
                            'izin_h_detail.no_rekening'
                        )
                        ->distinct()
                        ->join('izin_h', 'izin_h.kode_buku', '=', 'izin_h_detail.kode_buku')
                        ...
                        ->orWhere('izin_h_detail.no_rekening','like','%'.$request.'%')
                        ->groupBy(
                            'izin_h_detail.kode_perusahaan',
                            'perusahaans.nama_perusahaan',
                            'rekening_fin_comp.atas_nama_rek',
            ...
            if($total_row > 0)
            {
                ...
                {
                    $output .= '
                        <tr class="pilih_rekening" data-kodeperusahaan="'.$row->kode_perusahaan.'" data-namaperusahaan="'.$row->nama_perusahaan.'" data-atasnama="'.$row->atas_nama_rek.'" data-kodebank="'.$row->kode_bank.'" data-namabank="'.$row->nama_bank.'" data-norek="'.$row->no_rekening.'" >
        ...
        if($request->ajax())
        {
            ...
            if($query != ''){
                $data = DB::table('izin_h')
                    ...
                    ->where('izin_h_detail.no_rekening', $norek)
    ...
    public function action_seri_akhir(Request $request)
    {
        if($request->ajax())
        {
            $output = '';
            $query = $request->get('query');
            $norek_akhir = $request->get('norek_akhir');
            ...
            if($query != ''){
                $data = DB::table('izin_h')
                    ...
                    ->Where('izin_h_detail.status', 1)
                    ->where('izin_h_detail.no_rekening', $norek_akhir)
                    ->Where('izin_h_detail.id_cek','like','%'.$query.'%')
                    //->orderBy('izin_h_detail.id_cek','ASC')
                    ->get();
            }else{
            ...
            if($total_row > 0)
            {
                ...
                {
                    $output .= '
                        <tr class="pilih_seri_akhir" data-kode_buku="'.$row->kode_buku.'" data-no_izin="'.$row->no_izin.'" data-no_rek="'.$row->no_rekening.'" data-id_cek="'.$row->id_cek.'" data-no_cek="'.$row->no_cek.'">
                            <td>'.$row->kode_buku.'</td>
                            <td>'.$row->no_izin.'</td>
                            <td>'.$row->judul_izin.'</td>
                            <td>'.$row->kode_perusahaan.'</td>
    ...
    {
        $getRow = DB::table('izin_b')
            ->select(DB::raw('MAX(kode_izin_b) as NoUrut'));
        ...
        $kode_daftar = 'B '.$kode.''.'/'.''.$bulan_romawi.''.'/'.''.$tahun;

        $getRow_urut = DB::table('izin_b')->select(DB::raw('COUNT(kode_izin_b) as NoUrut'))->first();
        ...
        Izin_B::create([
            'kode_izin_b' => $kode_daftar,
            'tgl_izin_b' => Carbon::now()->format('Y-m-d'),
            'no_izin_b' => $request->get('no_izin'),
            'judul_izin_b' => $request->get('judul_izin'),
            'catatan_b' =>  $request->get('keterangan'),
            'rekening_pembayar' => $request->get('norek_pembayar'),
            'no_urut' => $no_urut,  
        ...
        foreach ($request->input('seri_awal') as $key => $value) {
            ...
            for($i = $request->get("seri_awal")[$key]; $i <= $request->get("seri_akhir")[$key]; $i++){
                ...
                $data = new Izin_B_Detail();
                $data->kode_izin_b = $kode_daftar;
                ...
                $data->seri_awal = $request->get("no_cek")[$key]; 
                $data->seri_akhir = $request->get("no_cek_akhir")[$key];
                $data->kode_vendor = $request->get("kode_vendor")[$key];
                $data->atas_nama = $request->get("nama_rekening")[$key];
                $data->kode_bank_vendor = $request->get("kode_bank_vendor")[$key];
                $data->no_rekening_vendor = $request->get("no_rek_vendor")[$key];
                $data->jml_lembar = 0;
        ...
        return redirect()->route('tanda_terima_b.index');
    }
